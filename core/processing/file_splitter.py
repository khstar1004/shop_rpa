...
class FileSplitter:
    ...
    def split_input_file(self, df: pd.DataFrame, input_file: str) -> List[str]:
        ...
        try:
            if not self.needs_splitting(df):
                self.logger.info(f"No need to split file with {len(df)} rows (threshold: {self.split_threshold})")
                return [input_file]
            ...
            self.logger.info(f"Splitting file into {num_files} parts (total rows: {total_rows})")
            ...
            for i in range(num_files):
                ...
                self.logger.info(f"Created split file: {split_path} with {chunk_size} rows")
    ...
    def merge_result_files(self, result_files: List[str], original_input: str) -> str:
        ...
        try:
            ...
            self.logger.info(f"Created merged file: {merged_filename} with {len(merged_df)} rows")
