...
class DataCleaner:
        ...
        try:
            ...
            if "상품Code" in df_copy.columns:
                for fallback in ["상품코드", "Code", "업체코드"]:
                    if fallback in df_copy.columns:
                        df_copy["상품Code"] = df_copy["상품Code"].fillna(
                            df_copy[fallback]
                        )

                df_copy["상품Code"] = df_copy["상품Code"].apply(
                    lambda x: self.clean_product_code(
                        x, auto_correct and "product_code" in rules
                    )
                )
            ...
            if price_col in df_copy.columns:
                ...
                df_copy[price_col] = df_copy[price_col].apply(
                    lambda x: self.clean_price(
                        x, min_price, max_price, auto_correct and "price" in rules
                    )
                )
    ...
    def clean_product_names(
        self, names: Union[pd.Series, str]
    ) -> Union[pd.Series, str]:
        """상품명 정제"""
        try:
    ...
    def clean_product_code(self, code: Any, auto_correct: bool) -> str:
        ...
        try:
            ...
            if not auto_correct:
                self.logger.debug(
                    f"Invalid product code: {code_str} (auto-correction disabled)"
                )
                return ""
            ...
            if cleaned_code and re.match(pattern, cleaned_code):
                self.logger.debug(
                    f"Auto-corrected product code: {code_str} -> {cleaned_code}"
                )
                return cleaned_code
            else:
                # 유효한 코드 생성
                ...
                self.logger.warning(
                    f"Could not correct product code: {code_str}, using fallback: {fallback_code}"
                )
    ...
    def clean_price(
        self, price: Any, min_price: float, max_price: float, auto_correct: bool
    ) -> float:
        """가격 정제"""
        try:
            if pd.isna(price):
            ...
            if not auto_correct:
                self.logger.warning(
                    f"Price out of range: {price_float} (min={min_price}, max={max_price})"
                )
                return 0.0
    ...
    def clean_url(self, url: Any, auto_correct: bool) -> str:
        ...
        try:
            ...
            if (
                auto_correct
                and url_str
                and not url_str.startswith(("http://", "https://"))
            ):
    ...
    def validate_excel_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            # 필수 컬럼 검사
            required_cols = self.excel_settings.get(
                "required_columns", ["상품명", "판매단가(V포함)"]
            )
            ...
            if "상품명" in df.columns:
                empty_names = df["상품명"].isna() | (
                    df["상품명"].astype(str).str.strip() == ""
                )
                if empty_names.any():
                    validation_errors.append(
                        f"Found {empty_names.sum()} rows with empty product names"
                    )
            ...
            if "판매단가(V포함)" in df.columns:
                ...
                if not invalid_prices.empty:
                    validation_errors.append(
                        f"Found {len(invalid_prices)} rows with invalid prices"
                    )
            ...
            for url_col in ["본사 이미지", "본사상품링크"]:
                if url_col in df.columns:
                    ...
                    if not invalid_urls.empty:
                        validation_errors.append(
                            f"Found {len(invalid_urls)} rows with invalid URLs in '{url_col}'"
                        )
