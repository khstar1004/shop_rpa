...
class ExcelManager:
    ...
    def _is_price_in_range(self, price) -> bool:
        ...
        try:
            ...
            min_price = float(self.excel_settings.get('validation_rules', {}).get('price', {}).get('min', 0))
            max_price = float(self.excel_settings.get('validation_rules', {}).get('price', {}).get('max', 1000000000))
    ...
    def filter_excel_by_price_diff(self, file_path: str) -> str:
        ...
        try:
            ...
            if isinstance(price_diff_condition, bool) and not price_diff_condition:
                self.logger.warning("No price difference columns found or no conditions specified.")
                return file_path
    ...
    def insert_image_to_excel(self, image_path: str, target_cell: str):
        """
        이미지를 엑셀 셀에 삽입합니다.
        ...
        """
        try:
            self.logger.info(f"Inserting image {image_path} to cell {target_cell}")
            ...
            try:
                ...
                self.logger.error("Required libraries missing. Install win32com.client and pillow.")
                return False
            ...
            if not os.path.exists(abs_image_path):
                self.logger.error(f"Image file not found: {abs_image_path}")
                return False
            ...
            if not re.match("^[A-Z]+[0-9]+$", target_cell):
                self.logger.error(f"Invalid cell address: {target_cell}")
                return False
            ...
            try:
                ...
                # 워크북 저장
                workbook.Save()
                self.logger.info(f"Image inserted successfully to cell {target_cell}")
                
                return True
            
            except Exception as e:
                self.logger.error(f"Error inserting image to Excel: {str(e)}")
                return False
                
        except Exception as e:
            self.logger.error(f"Error in insert_image_to_excel: {str(e)}", exc_info=True)
            return False 