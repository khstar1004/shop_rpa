...
class MultiModalMatcher:
    ...
    def find_matches(
        ...
        for candidate in candidate_products:
            ...
            if hasattr(candidate, "supplier_name") and self._contains_filter_keywords(
            ...
            # '품절' 상태 체크
            if (
                hasattr(candidate, "stock_status")
                and str(candidate.stock_status).strip() == "품절"
            ):
                filter_out_candidates.append(candidate.id)
                continue
        ...
        for candidate in candidate_products:
            ...
            if (
                source_product.id == candidate.id
                or candidate.id in filter_out_candidates
            ):
                continue
            ...
            if (
                not category_compatible
                and hasattr(source_product, "category")
                and source_product.category
            ):
                self.logger.debug(
                    f"Skipping product {candidate.name} due to incompatible category"
                )
                continue

            # Calculate text similarity
            text_similarity = self.text_matcher.calculate_similarity(
                source_product.name, candidate.name
            )

            # 제품 규격 추출 및 비교
            if hasattr(self.text_matcher, "extract_product_specs"):
                source_specs = self.text_matcher.extract_product_specs(
                    source_product.name
                )
                candidate_specs = self.text_matcher.extract_product_specs(
                    candidate.name
                )
                    ...
                    if spec_type in candidate_specs:
                        for s_val in source_values:
                            for c_val in candidate_specs[spec_type]:
                                if self.text_matcher._normalize_spec(
                                    s_val
                                ) == self.text_matcher._normalize_spec(c_val):
                                    spec_match = True
                                    break
                            if spec_match:
                                break
            ...
            if source_product.image_url and candidate.image_url:
                ...
                image_similarity = self.image_matcher.calculate_similarity(
                    source_product.image_url, candidate.image_url
                )
            ...
            if not has_both_images:
                ...
                if text_similarity >= 0.8:
                    image_similarity = (
                        0.7  # 텍스트가 매우 유사하면 이미지도 유사할 가능성 높음
                    )
            ...
            combined_similarity = (
                text_weight * text_similarity + image_weight * image_similarity
            )
            ...
            if text_similarity >= 0.9:
                self.logger.info(
                    f"High text similarity match found: {candidate.name} (Text: {text_similarity:.2f})"
                )
            ...
            if (
                hasattr(source_product, "price")
                and source_product.price > 0
                ...
                std_price_check = (
                    ...
                    or abs(price_difference_percent) >= self.min_price_diff_percent
                )
                ...
                if hasattr(candidate, "supplier_name"):
                    # 네이버 상품 타입 체크 로직
                    is_naver_product = (
                        "naver" in candidate.supplier_name.lower()
                        or hasattr(candidate, "is_naver")
                        and candidate.is_naver
                    )
            ...
            if not price_filter_passed:
                # 가격 차이가 작아서 필터링됨
                self.logger.debug(
                    f"Skipping product {candidate.name} due to insufficient price difference ({price_difference_percent:.1f}%)"
                )
                continue
