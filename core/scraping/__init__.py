        ...
        try:
            ...
            # Selenium 인터페이스
                ...
                if kwargs.get("multiple", False):
                    return source.find_elements(
                        kwargs.get("by", "css selector"), selector
                    )
                else:
                    return source.find_element(
                        kwargs.get("by", "css selector"), selector
                    )
            return None
...
class CoordinateExtractionStrategy(ExtractionStrategy):
    ...
    def can_handle(self, source, selector, **kwargs) -> bool:
        return hasattr(source, "get_screenshot_as_base64") and isinstance(
            selector, (tuple, list)
        )
                ...
                if kwargs.get("use_ocr", False):
                    try:
                        ...
                        return pytesseract.image_to_string(
                            cropped, lang=kwargs.get("lang", "kor+eng")
                        )
                    except ImportError:
                        logging.warning(
                            "Tesseract OCR not available. Returning image only."
                        )
                        return cropped
                else:
                    return cropped
...
class BaseMultiLayerScraper:
    ...
    def __init__(
        self, max_retries: int = 3, timeout: int = 30, cache: Optional[FileCache] = None
    ):
    ...
    def set_cached_result(self, url, selector_key, value, ttl=None):
        ...
        if self.cache:
            ...
            try:
                ...
                logging.error(
                    f"Failed to cache results for key '{cache_key}': {str(e)}"
                )
    ...
    async def extract_async(self, source, selectors, **kwargs):
        ...
        for selector_key, selector_info in selectors.items():
            ...
            # 적절한 전략 결정
            strategy = self._get_extraction_strategy(
                source, selector, **selector_kwargs
            )
            if not strategy:
                continue
        ...
        for selector_key, task in tasks:
            try:
                ...
                self.logger.warning(
                    f"Extraction timed out for selector: {selector_key}"
                )
    ...
    async def _extract_with_timeout(self, strategy, source, selector, kwargs):
        ...
        return await asyncio.wait_for(
            loop.run_in_executor(
                self.executor, lambda: strategy.extract(source, selector, **kwargs)
            ),
    ...
    def add_sparse_data(self, key, value, ttl=None):
        ...
        self.sparse_data[key] = {"value": value, "expire_at": expire_time}
    ...
    def clean_expired_data(self):
        ...
        keys_to_remove = [
            key
            for key, data in self.sparse_data.items()
