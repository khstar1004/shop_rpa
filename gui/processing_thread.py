...
class ProcessingThread(QThread):
    ...
    def _process_single_file(self, input_file):
        ...
        try:
            ...
            if hasattr(self.processor, 'process_file'):
                # 기존 API 호환성
                primary_report_path, secondary_report_path = self.processor.process_file(input_file)
                if self._is_running:
                ...
                if self.product_limit:
                    self.status_updated.emit(f"최대 {self.product_limit}개 상품만 처리합니다...")
                    result_file = self.processor._process_limited_file(input_file, self.output_dir, self.product_limit)
                else:
    ...
    def _process_multiple_files(self):
        ...
        try:
            ...
            # 모든 파일 처리
            output_files = self.processor.process_files(self.input_files, self.output_dir, self.product_limit)
